...
The system SHALL use SQLite persistence when a `Peers` connection string is configured. When no connection string is present, the system SHALL fall back to the in-memory implementation. The connection string value SHALL support both relative paths (development) and absolute paths (production/Azure).
